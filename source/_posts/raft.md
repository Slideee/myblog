...
要开始一次选举，跟随者要先增加任期并切换到候选人状态。然后会向集群中的其他节点发送RPC请求来给自己投票，直到领导人被选举成功。

在等待投票时，候选人可能从其他服务器接收到声明它是领导人的请求。如果这个领导人的人气好不小于候选人的任期号，候选人承认该领导人并退回跟随者。反之则会拒绝。

如果候选人没有赢得选举也没有输，即多个跟随者成为候选人，那么选票会被瓜分，此时通过超时机制，并增加任期号来开始新的一轮的选举。这里的选举超时时间是从一个固定的区间（例如150-300毫秒）随机选择。
...
![raft6](/images/posts/raft/raft-图6.png)
...
具体的做法是，领导人会为每个跟随者维护一个nextIndex，表示下一个需要发送给跟随者的日志索引地址。当一个领导人刚被选举完成时，它会初始化所有nextIndex为自己最后一条的日志index加1。如果领导人和跟随者的日志不一致，一致性检查就会失败。此时领导人通过减小nextIndex，一直重试到和跟随者日志达成一致为止。这时就会把跟随者在这个index之后所有有冲突的条目删除并添加领导人的日志。领导人从来不会覆盖或者删除自己的日志。
